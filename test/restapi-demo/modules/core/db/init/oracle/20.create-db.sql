...
alter table REF_ALLOCATED_CAR add constraint FK_REF_ALLOCATED_CAR_ON_MODEL foreign key (MODEL_ID) references REF_MODEL(ID)^
create index IDX_REF_ALLOCAR_ON_MODEL on REF_ALLOCATED_CAR (MODEL_ID)^
...
alter table REF_CAR_TOKEN add constraint FK_REF_CARTOKE_ON_GARATOKE foreign key (GARAGE_TOKEN_ID) references REF_CAR_GARAGE_TOKEN(ID)^
create index IDX_REF_CAR_TOKEN_ON_REPAIR on REF_CAR_TOKEN (REPAIR_ID)^
create index IDX_REF_CARTOK_ON_GARTOK on REF_CAR_TOKEN (GARAGE_TOKEN_ID)^
-- end REF_CAR_TOKEN
...
create index IDX_REF_DRIVALLO_ON_DRIVER on REF_DRIVER_ALLOC (DRIVER_ID)^
create index IDX_REF_DRIVER_ALLOC_ON_CAR on REF_DRIVER_ALLOC (CAR_ID)^
-- end REF_DRIVER_ALLOC
-- begin REF_IK_ORDER_LINE_TAG
alter table REF_IK_ORDER_LINE_TAG add constraint FK_REF_IKORLITA_ON_ORDER_LINE foreign key (ORDER_LINE_ID) references REF_IK_ORDER_LINE(ID)^
create index IDX_REF_IKORLITA_ON_ORLI on REF_IK_ORDER_LINE_TAG (ORDER_LINE_ID)^
...
create index IDX_REF_IKORDLIN_ON_ORDER on REF_IK_ORDER_LINE (ORDER_ID)^
...
alter table REF_CAR_DETAILS_ITEM add constraint FK_REF_CARDETITE_ON_CARDET foreign key (CAR_DETAILS_ID) references REF_CAR_DETAILS(ID)^
create index IDX_REF_CARDETITE_ON_CARDET on REF_CAR_DETAILS_ITEM (CAR_DETAILS_ID)^
...
alter table REF_DRIVER add constraint FK_REF_DRIVE_ON_PLATFENTIT foreign key (PLATFORM_ENTITY_ID) references REF_SAMPLE_PLATFORM_ENTITY(ID)^
...
create index IDX_REF_DRIV_ON_DRIVER_GROUP on REF_DRIVER (DRIVER_GROUP_ID)^
create index IDX_REF_DRIV_ON_PLATENTI on REF_DRIVER (PLATFORM_ENTITY_ID)^
-- end REF_DRIVER
...
alter table REF_REPAIR add constraint FK_REF_REPAI_ON_INSURCASE foreign key (INSURANCE_CASE_ID) references REF_INSURANCE_CASE(ID)^
create index IDX_REF_REPAIR_ON_CAR on REF_REPAIR (CAR_ID)^
create index IDX_REF_REPA_ON_INSUCASE on REF_REPAIR (INSURANCE_CASE_ID)^
-- end REF_REPAIR
-- begin REF_PRICING_REGION
alter table REF_PRICING_REGION add constraint FK_REF_PRICIREGIO_ON_PARENT foreign key (PARENT_ID) references REF_PRICING_REGION(ID)^
create index IDX_REF_PRICREGI_ON_PARENT on REF_PRICING_REGION (PARENT_ID)^
...
alter table REF_DRIVER_LICENSE add constraint FK_REF_DRIVELICEN_ON_DRIVER foreign key (DRIVER_ID) references REF_DRIVER(ID)^
alter table REF_DRIVER_LICENSE add constraint FK_REF_DRIVER_LICENSE_ON_CAR foreign key (CAR_ID) references REF_CAR(ID)^
create index IDX_REF_DRIVLICE_ON_DRIVER on REF_DRIVER_LICENSE (DRIVER_ID)^
create index IDX_REF_DRIVER_LICENSE_ON_CAR on REF_DRIVER_LICENSE (CAR_ID)^
-- end REF_DRIVER_LICENSE
-- begin REF_DRIVER_CALLSIGN
create unique index IDX_REF_DRIVCALL_UK_CALLSIGN on REF_DRIVER_CALLSIGN (CALLSIGN, DELETE_TS) ^
...
alter table REF_CARD add constraint FK_REF_CARD_ON_SUBSTCREAT foreign key (SUBSTITUTED_CREATOR_ID) references SEC_USER(ID)^
...
create index IDX_REF_CARD_ON_SUBSCREA on REF_CARD (SUBSTITUTED_CREATOR_ID)^
...
alter table REF_CAR add constraint FK_REF_CAR_ON_CARDOCUM foreign key (CAR_DOCUMENTATION_ID) references REF_CAR_DOCUMENTATION(ID)^
...
create index IDX_REF_CAR_ON_CARDOCU on REF_CAR (CAR_DOCUMENTATION_ID)^
...
alter table REF_PLANT_MODEL_LINK add constraint FK_PLAMOD_ON_PLANT foreign key (PLANT_ID) references REF_PLANT(ID)^
alter table REF_PLANT_MODEL_LINK add constraint FK_PLAMOD_ON_MODEL foreign key (MODEL_ID) references REF_MODEL(ID)^
